...
def build_rpnTarget(boxes, anchors, config):
    ...
    max_iou = iou[np.arange(iou.shape[0]), maxArg_iou] #算出iou，根据iou来设置label
    ...
    for i, a in zip(idxs, anchors[idxs]):
        ...
        rpn_bboxes[ix] /= config.RPN_BBOX_STD_DEV #除以一个小数，来放大偏差，这样有利于训练，值太小了会不利于训练
        ix += 1
